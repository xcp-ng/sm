...
class LinstorSR(SR.SR):
    ...
    def _load_vdis_ex(self):
        ...
        try:
            self.vdis = {vdi.uuid: vdi for vdi in self._multi_cowutil.run(load_vdi, vdi_uuids)}
        finally:
            multi_cowutil = self._multi_cowutil
            self._multi_cowutil = None
            del multi_cowutil
    ...
    def _load_this(self):
        ...
        if not VdiType.isCowImage(self.vdi_type):
            ...
            if self.sr._multi_cowutil:
                ...
                cowutil_instance = self.linstorcowutil

            image_info = cowutil_instance.get_info(self.uuid)
