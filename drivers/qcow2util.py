...
class QCowUtil(CowUtil):
    ...
    QCOW2_INCOMPATIBLE_FEATURE_EXTENDED_L2 = 0x10
    ...
    def _read_qcow2(self, path: str, read_clusters: bool = False):
        ...
        with open(path, "rb") as qcow2_file:
            ...
            if read_clusters: #TODO(XCPNG-3502): We want to remove this code completely and have everything that need to read allocation table be done in qcow2_helper
    ...
    def _get_l2_entries(file: BinaryIO, l2_offset: int) -> List[int]:
        ...
        extended_l2 = bool(self.header["incompatible_features"] & self.QCOW2_INCOMPATIBLE_FEATURE_EXTENDED_L2)
    ...
    def _convert_bool_to_qemu_option_value(self, option: bool) -> str:
    ...
    @override
    def getExtendedL2(self, path: str) -> bool:
        self._read_qcow2(path)
        return bool(self.header["incompatible_features"] & self.QCOW2_INCOMPATIBLE_FEATURE_EXTENDED_L2)
    ...
    def calcOverheadBitmap(self, virtual_size: int) -> int:
        """
        We are not impacted, this is only for VHD. We send back 0 for QCOW2.
        """
        return 0
    ...
    def getInfo(
        ...
        """
        resolveParent is useful to find the absolute path of a parent since the path stored in VHD is relative.
        For linstor, this cause issues since the parent might not exist locally.
        We currently do not implement this for QCOW2.
        """
        self._read_qcow2(path)
    ...
    def _getAllInfoFromPath(
        ...
        result: Dict[str, CowImageInfo] = dict()
        #TODO(XCPNG-3504): Use qcow2_helper for scan of FileSR and add handling of exitOnError
    ...
    def setSizePhys(self, path: str, size: int, debug: bool = True) -> None:
        pass #We do not store the physical size in the image in QCOW2
    ...
    def snapshot(
        ...
        if parentRaw:
            ...
            extended_l2 = self.getExtendedL2(parent)
    ...
    def _parseQCOW2Info(line: str, extractUuidFunction: Callable[[str], str]) -> Optional[CowImageInfo]:
        ...
        for keyval in valueMap:
            ...
            if key == "scan-error": #TODO(XCPNG-3503): need to add this to scan from qcow2_helper
                vhdInfo.error = line
