...
class QCowUtil(CowUtil):
    ...
    def setParent(self, path: str, parentPath: str, parentRaw: bool) -> None:
        ...
        if parentRaw:
            parentType = RAW_TYPE
        cmd = [QEMU_IMG, "-q", "rebase", "-u", "-f", QCOW2_TYPE, "-F", parentType, "-b", parentPath, path]
        self._ioretry(cmd)
    ...
    def setSizeVirt(self, path: str, size: int, jFile: str) -> None:
        ...
        """
        cmd = [QEMU_IMG, "-q", "resize", path, str(size)]
        self._ioretry(cmd)
    ...
    def create(self, path: str, size: int, static: bool, msize: int = 0, block_size: Optional[int] = None) -> None:
        cmd = [QEMU_IMG, "-q", "create", "-f", QCOW2_TYPE, path, str(size)]
        if static:
    ...
    def repair(self, path: str) -> None:
        cmd = [QEMU_IMG, "-q", "check", "-f", QCOW2_TYPE, "-r", "all", path]
        self._ioretry(cmd)
