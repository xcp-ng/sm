...
class FileSR(SR.SR):
    ...
    def _has_journals(self) -> bool:
        for key in self.JRN_KEYS:
            try:
                if any(self.journaler.getAll(key)):
                    return True
            except OSError:
                # If we can't read the SR, this means there isn't any journal
                # and/or didn't finish it's creation steps
                return False
        return False
    ...
    def _loadvdis(self):
        ...
        for vdi_type in VDI_COW_TYPES:
            ...
            try:
                image_info = cowutil.getAllInfoFromVG(pattern, FileVDI.extractUuid)
            except util.CommandException as inst:
            ...
            try:
                ...
                if len(image_info) != len(vdi_uuids):
                    util.SMlog("VDI scan of %s returns %d VDIs: %s" % (extension, len(image_info), sorted(image_info)))
        ...
        for uuid, image_info in self.image_info.items():
            if image_info.error:
            ...
            vdi_path = os.path.join(self.path, image_info.path)
    ...
    def _rollback_revert_vdi(self, entry: RevertLogEntry):
        ...
        self._unlink_paths(entry.dest.path)
...
class FileVDI(VDI.VDI):
        ...
        for try_index in range(maxretry):
            for vdi_extension, vdi_type in vdi_formats:
                ...
                if util.ioretry(lambda: util.pathexists(vdi_path)):
                    return vdi_path, vdi_type

            util.SMlog(f"VDI {vdi_uuid_str} not found, try {try_index + 1} of {maxretry}")
            if try_index + 1 < maxretry:
                time.sleep(period)

        return None, None

    def _resolve_vdi_context_from_path(self, vdi_uuid, maxretry=5, period=2.0) -> bool:
    ...
    def _revert(
        ...
        dest_backup_path = os.path.join(
            dest.sr.path,
            f"{util.gen_uuid()}{VDI_TYPE_TO_EXTENSION[dest.vdi_type]}",
        )
    ...
    def load_from_file(self, path: Union[str, Path]):
        ...
        self.path = str(path)
        image_info = self.cowutil.getInfo(self.path, FileVDI.extractUuid)
        self.utilisation = image_info.sizePhys
        self.size = image_info.sizeVirt
        self.parent = image_info.parentUuid
        self.hidden = image_info.hidden
    ...
    def _snapshot(self, snap_type, cbtlog=None, cbt_consistency=None, is_mirror_destination=False):
        ...
        if snap_type == VDI.SNAPSHOT_DOUBLE:
            dest = util.gen_uuid()
            dst = os.path.join(self.sr.path, dest + extension)
            args.append(dest)
        ...
        src = self.path
        newsrcname = newuuid + extension
        newsrc = os.path.join(self.sr.path, newsrcname)
    ...
    def _delete_cbt_log(self) -> None:
        ...
        if not self._cbt_log_exists(logPath):
            util.SMlog(f"CBT delete: {logPath} not found, skipping")
            return
        try:
            os.remove(logPath)
        except OSError as e:
