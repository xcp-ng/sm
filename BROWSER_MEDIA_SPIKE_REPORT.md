...
| --- | --- |
| `drivers/BrowserISOSR.py` — new | Implements the `browseriso` backend, temporary read-only VDI metadata, endpoint checks, and the nbdkit/tapdisk lifecycle. Returns the existing NBD attachment representation. |
| `drivers/BrowserISOSR` — generated | Makefile installs the usual driver entry-point symlink to `BrowserISOSR.py`. |
| `drivers/SRCommand.py` | Adds an explicit opt-in path for attachment/detachment managed by the driver itself. This avoids wrapping its NBD endpoint as a conventional file-backed disk. Also redacts the driver's source configuration from normal command logging. |
| `drivers/blktap2.py` | Adds `nbd` to the recognized tapdisk types so SM can enumerate the new backend. The tapdisk NBD client itself already existed. |
